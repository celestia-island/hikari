...
dev 分支将 `:hover` CSS 规则替换为 AnimationBuilder 命令式驱动（通过 `--glow-intensity-scale`）。**方案 A（最小化恢复）已完成**：event handler 现在通过 `set_style_property` 正确设置 `--glow-intensity-scale`，实现与 master 分支等效的动画效果。
...
| hover 触发 | CSS `:hover::before { opacity }` | JS 设置 `--glow-intensity-scale` | ✅ 已实现（方案 A） |
| `onmouseenter` | 无（CSS `:hover` 处理） | 设置 `--glow-intensity-scale: 0.5` | ✅ 已实现 |
| `onmouseleave` | 无（CSS `:hover` 处理） | 设置 `--glow-intensity-scale: 0` | ✅ 已实现 |
| `onmousedown` | 无 | 设置 `--glow-intensity-scale: 1.0` | ✅ 已实现 |
| `onmouseup` | 无 | 设置 `--glow-intensity-scale: 0.5` | ✅ 已实现 |
...
| 功能 | 状态 | 说明 |
|------|------|------|
| `use_element_ref` / DOM ref 绑定 | ✅ 已实现 | `ElementRef` + `use_element_ref` hook |
| `UseAnimation` rAF 集成 | ✅ 已实现 | `AnimationHandle` + `request_animation_frame` |
| `EasingFunction::evaluate(t)` | ✅ 已实现 | 完整的缓动函数数学计算 |
| `MouseEvent` 字段（offset_x/y, page_x/y, movement_x/y） | ✅ 已实现 | WIT 定义 + builder 方法 |
| `WheelEvent`/`TouchEvent`/`PointerEvent` | ✅ 已实现 | 完整事件支持 |
| `TransitionEvent`/`AnimationEvent` | ✅ 已实现 | 完整事件支持 |
| Signal → 重渲染调度 | ⚠️ 长期 | `use_signal`/`use_state` 变化不触发重渲染 |
| `apply_patch` | ⚠️ 长期 | diff 计算后无法增量更新 DOM |
...
```
onmouseenter → set_style_property(&wrapper, "--glow-intensity-scale", "0.5")
onmouseleave → set_style_property(&wrapper, "--glow-intensity-scale", "0")
onmousedown  → set_style_property(&wrapper, "--glow-intensity-scale", "1.0")
onmouseup    → set_style_property(&wrapper, "--glow-intensity-scale", "0.5")（如仍 hover）
```

**实现细节**：
- 使用 `event.client_x/y` 正确获取事件坐标
- 通过 `element_from_point(client_x, client_y)` 获取目标元素
- 通过 `get_element_by_class_upward("hi-glow-wrapper", &target_el)` 获取 wrapper 元素
- 使用 `set_style_property` 设置 CSS 变量
...
| 优先级 | 任务 | 依赖 | 状态 |
|--------|-----|------|------|
| **P0** | ✅ 方案 A：在 handler 中直接 `set_style_property` 设置 `--glow-intensity-scale` | 已完成 | 已完成 |
| **P0** | ✅ 验证 tairitsu `use_element_ref` | tairitsu 侧实现 | 已验证 |
| **P0** | ✅ 验证 tairitsu `UseAnimation` rAF 集成 | tairitsu 侧实现 | 已验证 |
| **P0** | ✅ 验证 `EasingFunction::evaluate(t)` | tairitsu 侧实现 | 已验证 |
| **P0** | ✅ SSR 零改动验证 | tairitsu-ssr 完成 | 已完成 |
| **P1** | 方案 B：Glow + ButtonStateMachine + AnimationBuilder 集成 | 无 | 已分析 |
| **P2** | ✅ `MouseEvent` 补充 `offset_x/y` 字段 | tairitsu 侧实现 | 已完成 |
| **P2** | ✅ `GlowAnimation` 预设（pulse/breathe/shimmer）联调 | 无 | 已完成 |
...
| 标准 | 状态 | 说明 |
|------|------|------|
| 1. 鼠标进入 Glow Wrapper 区域，`::before` 伪元素 opacity 从 0 平滑过渡到可见 | ✅ 代码已实现 | 需要实际运行网站验证视觉效果 |
| 2. 鼠标在区域内移动，径向渐变中心跟随鼠标实时更新 | ✅ 代码已实现 | `onmousemove` 已正确实现 |
| 3. 鼠标离开区域，glow 效果平滑淡出 | ✅ 代码已实现 | `onmouseleave` 设置 `--glow-intensity-scale: 0` |
| 4. Button/Card/Input 等使用 Glow 的组件均表现正常 | ✅ 代码已实现 | 所有组件使用相同的 Glow 逻辑 |
| 5. SSR 场景不报错 | ✅ 代码已实现 | `#[cfg(not(wasm32))]` 分支正常渲染无 glow 的 div |
...
| 优先级 | 任务 | 状态 |
|--------|-----|------|
| P0 | ✅ tairitsu-ssr 完成 | 已完成 |
| P0 | ✅ 零改动 SSR 验证 | 已完成 |
| P1 | SSR 开发服务器集成测试 | 待实现 |
| P2 | Hydration 兼容验证 | 待实现 |
| P3 | 路由级 SSR | 长期 |
