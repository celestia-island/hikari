...
|------|-------------|-------------------|---------|
| avatar | `"hi-avatar"`, `"hi-avatar-icon"`, `"hi-avatar-fallback"`, `"hi-avatar-img"` | ✅ AvatarClass 已创建 | 已修复 ✅ |
| background | `"hi-background"` | ✅ BackgroundClass 已创建 | 已修复 ✅ |
| canvas | `"hi-canvas"` | ❌ 无 | 简单组件，无需专用枚举 |
| image | `"hi-image"`, `"hi-image-container"`, `"hi-image-placeholder"` | ✅ ImageClass 已创建 | 已修复 ✅ |
| modal | `"hi-modal"` 等 | ✅ ModalClass 已存在 | 已修复 ✅ |
| collapse | 未使用 Palette | ✅ CollapseClass 已存在 | 已修复 ✅ |
| tree | 未使用 Palette | ✅ TreeClassNew 已存在 | 已修复 ✅ |
| skeleton | 通用 `Display, FlexDirection, Gap, Padding` | ✅ SkeletonClass 已存在 | 已修复 ✅ |
| popover | 仅 `Display`, `Position` | ✅ PopoverClass 已存在 | 已修复 ✅ |
...
| 组件 | 当前导入 | 应改为 | 状态 |
|------|---------|--------|------|
| `data/drag.rs` | `TreeClass` | 新建 `DragClass` 或使用 `DragDropTreeClass` | 已修复 ✅ |
| `data/virtual_scroll.rs` | `TreeClass` | `VirtualScrollClass`（已在 Palette 中定义） | 已修复 ✅ |
| `display/skeleton.rs` | 通用 `Display, FlexDirection, Gap, Padding` | 补充 `SkeletonClass` 导入 | 已修复 ✅ |
| `basic/textarea.rs` | `InputClass` | 可接受（共享输入样式） | 已验证 ✅ |
| `basic/icon_button.rs` | `ButtonClass` | 可接受（共享按钮样式） | 已验证 ✅ |

---

### D. Palette 中存在但组件未使用/无对应组件的枚举

| 枚举名 | 所在文件 | 状态 |
|--------|---------|------|
| `SpotlightClass` | display.rs | ❌ 无对应组件（添加 #[allow(dead_code)]） |
| `DescriptionListClass` | display.rs | ❌ 无对应组件（添加 #[allow(dead_code)]） |
| `DropdownClass` | feedback.rs | ✅ 已被 Portal 系统使用 |
| `RowClass` | layout.rs | ✅ 已被 Grid 组件使用 |
| `TableHeaderClass` | data.rs | ❌ 未使用（添加 #[allow(dead_code)]） |
| `TreeClassNew` | data.rs | ✅ 已被 Tree 组件使用 |
| `TreeLabelClass` | data.rs | ❌ 未使用（添加 #[allow(dead_code)]） |
| `DragDropTreeClass` | data.rs | ✅ 已被 Drag 组件使用 |
| `PortalClass` | misc.rs | ✅ 已被 Portal 系统使用 |
...
| 任务 | 影响 | 工作量 | 状态 |
|------|------|--------|------|
| 修复 Tabs 类名不匹配（修改 `TabsClass::as_suffix()`） | Tabs 组件完全无样式 | 小 | ✅ 已完成 |
| Modal 导入 ModalClass | Modal 丢失结构类 | 小 | ✅ 已完成 |
| Collapse 导入 CollapseClass | Collapse 丢失结构类 | 小 | ✅ 已完成 |
| Tree 导入 TreeClass | Tree 丢失结构类 | 小 | ✅ 已完成 |
| virtual_scroll 改用 VirtualScrollClass | VirtualScroll 类名错误 | 小 | ✅ 已完成 |
| drag.rs 修正 Palette 引用 | drag 类名错误 | 小 | ✅ 已完成 |

#### P1 — 重要修复（覆盖度提升）

| 任务 | 影响 | 工作量 | 状态 |
|------|------|--------|------|
| 全面对比每个组件的 SCSS 选择器 vs Palette 枚举 vs rsx! 输出 | 发现更多类名不匹配 | 大 | 进行中 |
| 为 Avatar、Image、Background 新建 Palette 枚举 | 硬编码组件获得类型安全 | 中 | ✅ 已完成 |
| 补充 Menu/Breadcrumb/Card/Select 等的缺失 Palette 变体 | 增加样式覆盖 | 中 | ✅ 已完成 |
| 修复 build.rs 添加 load_paths | 组件级 CSS 编译正确 | 小 | ✅ 已完成 |
| Layout 组件族添加 StyledComponent 实现 | 布局组件可注入样式 | 中 | ✅ 已完成 |
| 修复未使用导入警告 | 代码整洁 | 小 | ✅ 已完成 |

#### P2 — 功能增强

| 任务 | 影响 | 工作量 | 状态 |
|------|------|--------|------|
| 添加尺寸类支持（sm/md/lg）到核心组件 | 响应式设计 | 大 | ✅ Alert/Modal 已完成，Input/Button/Select/Table 已有 |
| 添加状态类（error/success）到表单组件 | 表单验证反馈 | 大 | ✅ Input/Textarea 已完成 |
| 在 tairitsu 新建 `tairitsu-i18n` 包 | i18n 基础设施 | 中 | ✅ 已完成 |
| hikari_i18n 集成 tairitsu_i18n | i18n 统一 | 小 | ✅ 已完成 |
| 清理未使用的 Palette 枚举 | 代码整洁 | 小 | ✅ 已添加 #[allow(dead_code)] |
| Skeleton 导入 SkeletonClass | 类型安全 | 小 | ✅ |
| Popover 导入 PopoverClass | 类型安全 | 小 | ✅ |

#### P3 — 技术债务

| 任务 | 影响 | 工作量 | 状态 |
|------|------|--------|------|
| 统一所有组件使用 `#[define_props]` | Props 系统一致性 | 中 | ✅ 66 个组件已迁移 |
| 为缺少 SCSS 的组件补充样式文件 | 样式完整性 | 大 | ✅ 新增 4 个 SCSS 文件 |
...
|------|------|
| 组件总文件数 | 72 |
| 有 StyledComponent 实现 | 69（96%） |
| 使用 Palette 专用枚举 | 62（86%） |
| 有对应 SCSS 文件 | 63 个（新增 4 个） |
| 使用 `#[define_props]` | 66+（92%） |
| 使用 i18n | 0（0%）- tairitsu-i18n 已就绪 |
| 类名完全匹配（确认） | ~25 个 |
| 类名不匹配或未审计 | ~10 个 |
| 无 Palette 且硬编码 | 0 个（已全部修复） |
...
| `basic/avatar.rs` | `styles/components/avatar.scss` ✅ | ✅ `AvatarComponent` | ✅ `AvatarClass` | ❌ | 裸 `#[component]` |
| `basic/background.rs` | `styles/components/background.scss` ✅ + `basic/styles/background.scss` | ✅ `BackgroundComponent` | ✅ `BackgroundClass` | ❌ | 🏚️ `#[derive(Props)]` |
...
| `basic/image.rs` | `styles/components/image.scss` ✅ | ✅ `ImageComponent` | ✅ `ImageClass` | ❌ | 裸 `#[component]` |
...
| `feedback/modal.rs` | `styles/components/modal.scss` ✅ | ✅ `ModalComponent` | ✅ `ModalClass` | ❌ | 裸 `#[component]` |
...
| `data/collapse.rs` | `styles/components/collapse.scss` ✅ | ✅ `CollapseComponent` | ✅ `CollapseClass` | ❌ | 🏚️ `#[derive(Props)]` |
| `data/column.rs` | ❌ | ❌ | ❌ | ❌ | 🏚️ `#[derive(Props)]` |
| `data/drag.rs` | `styles/components/drag.scss` ✅ | ✅ `DragComponent` | ✅ `DragDropTreeClass` | ❌ | 🆕 `#[define_props]` |
...
| `data/virtual_scroll.rs` | `styles/components/virtual-scroll.scss` ✅ | ✅ `VirtualScrollComponent` | ✅ `VirtualScrollClass` | ❌ | 🆕 `#[define_props]` |

### layout/ — 布局组件

| 组件文件 | SCSS 文件 | StyledComponent | Palette Class Enum | i18n | Props 风格 |
|----------|-----------|:---------------:|-------------------:|:----:|:----------:|
| `layout/app_layout.rs` | `styles/components/layout.scss` ✅ | ✅ `AppLayoutComponent` | `AppLayoutClass`, `Layout` | ❌ | 裸 `#[component]` |
| `layout/aside.rs` | `styles/components/aside.scss` ✅ | ✅ `AsideComponent` | `AsideClass` (via `components::*`) | ❌ | 裸 `#[component]` |
...
| `layout/grid.rs` | `styles/components/grid.scss` ✅ | ✅ `GridComponent` | `GridClass` (via `components::*`) | ❌ | 裸 `#[component]` |
| `layout/header.rs` | `styles/components/header.scss` ✅ | ✅ `HeaderComponent` | `Header` (via `classes::*`) | ❌ | 裸 `#[component]` |
| `layout/scrollbar.rs` | `styles/components/scrollbar_container.scss` ✅ | ✅ `ScrollbarComponent` | ❌ | ❌ | 裸 `#[component]` |
| `layout/section.rs` | `styles/components/section.scss` ✅ | ✅ `SectionComponent` | `SectionClass` | ❌ | 裸 `#[component]` |
...
| `AvatarClass` | 4 | Avatar, Icon, Fallback, Img |
| `ImageClass` | 3 | Image, Container, Placeholder |
| `BackgroundClass` | 1 | Background |
...
| SCSS 文件 | 组件 | 说明 |
|-----------|------|------|
| ~~`avatar.scss`~~ | ~~`basic/avatar.rs`~~ | ~~无 StyledComponent、无 Palette 导入~~ ✅ 已修复 |
| ~~`image.scss`~~ | ~~`basic/image.rs`~~ | ~~无 StyledComponent、无 Palette 导入~~ ✅ 已修复 |
...
| ~~`TreeClass`~~ | ~~`hi-drag-node`, `hi-virtual-tree` 等~~ | ~~`tree.scss` + `drag.scss`~~ | ~~⚠️ `data/drag.rs` 和 `data/virtual_scroll.rs` 共用 `TreeClass`~~ ✅ 已修复 |
| ~~`ModalClass`~~ | ~~`hi-modal-*`~~ | ~~`modal.scss`~~ | ~~⚠️ `feedback/modal.rs` 未导入 `ModalClass`，硬编码类名~~ ✅ 已修复 |
