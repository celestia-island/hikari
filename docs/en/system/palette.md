...
```rust
// yes Type-safe - compile-time checking
let classes = ClassesBuilder::new()
 .add(Display::Flex)
 .add(FlexDirection::Row)
 .build();
...
```rust
// yes Good - Type-safe utilities
let classes = ClassesBuilder::new()
 .add(Display::Flex)
 .add(Gap::Gap4)
 .build();

// yes Acceptable - String-based (less type-safe)
