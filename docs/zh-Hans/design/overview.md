...
## 双层包架构：components 与 extra-components

Hikari 将组件体系拆分为两个互补的包，分别承担不同层次的职责：

```mermaid
graph LR
    subgraph "hikari-components"
        RC["渲染组件<br/>(rsx! + 响应式钩子 + StyledComponent)"]
    end
    subgraph "hikari-extra-components"
        DM["数据模型<br/>(纯结构体 + serde + builder)"]
    end
    RC -->|读取状态| DM
    DM -->|驱动渲染| RC
```
...
| 维度 | `hikari-components` | `hikari-extra-components` |
|------|---------------------|---------------------------|
| **渲染方式** | `rsx!` 宏、响应式钩子 | 无（框架无关） |
| **状态管理** | `use_signal()`、`use_effect()` | 纯可变结构体字段 |
| **事件处理** | `EventHandler<T>` 闭包 | `data-action` 属性 + 外部绑定 |
| **CSS 嵌入** | `StyledComponent` trait | 导出 `pub const *_STYLES` |
| **序列化** | 不需要 | 所有状态类型均派生 `serde` |
| **DOM 依赖** | 需要 Tairitsu 框架 | 无 |
| **适用场景** | Tairitsu 应用内的实时 UI 渲染 | SSR、测试、状态持久化、非 Tairitsu 框架 |

### 重叠组件域

以下组件在两个包中均存在，这是**有意设计**而非冗余：
...
`components` 版本提供**即用型渲染组件**（含动画、键盘处理、图标集成、StyledComponent CSS）；
`extra-components` 版本提供**纯数据模型**（含 builder 模式、serde 序列化、变更方法、单元测试）。

### 何时使用哪个包

- **Tairitsu 应用**：使用 `hikari-components` 进行 UI 渲染；可选使用 `hikari-extra-components` 进行状态持久化或撤销/重做
- **非 Tairitsu 应用**：使用 `hikari-extra-components` 的数据模型并自行实现渲染
- **测试**：使用 `hikari-extra-components` 在无 DOM 环境下进行状态逻辑单元测试
- **SSR**：两者并用——数据模型用于服务端状态，渲染组件用于客户端注水

### 类型消歧

部分类型在两个包中同名（如 `TimelinePosition`、`GuideStep`）。请使用显式模块路径导入：

```rust,ignore
use hikari_extra_components::extra::TimelineState;     // 纯数据模型
use hikari_components::display::Timeline;              // 渲染组件

use hikari_extra_components::extra::ZoomControlsState; // 纯状态
use hikari_components::display::ZoomControls;          // 渲染组件
```

### CSS 类名

两个包对同一概念元素使用不同的 CSS 类名。这是有意为之——`components` 使用来自 `hikari-palette` 的类型化类枚举（如 `ZoomControlsClass::Button`），而 `extra-components` 使用硬编码字符串或计算方法。当两个包同时使用时，各自以其自身的类集渲染。
