...
- [設計理念](#設計理念)
- [分層架構](#分層架構)
- [Layer 1：基礎組件](#layer-1基礎組件)
- [Layer 2：複合組件](#layer-2複合組件)
- [Layer 3：生產級組件](#layer-3生產級組件)
- [開發優先級](#開發優先級)
- [技術說明](#技術說明)
...
**範例**：Button、Input、Card、Badge、Alert、Toast、Tooltip
...
**範例**：Menu、Tabs、Table、Tree、Form、Dropdown、Modal

### Layer 3：生產級組件

**定義**：基於 Layer 2 構建的完整業務功能
...
**範例**：影片/音訊播放器、富文本編輯器、代碼高亮設施、時間軸、用戶引導組件
...
| **代碼編輯器** | 低 | 完整代碼編輯、智能提示、除錯 | Card, Tabs, Form, Menu | 高 |
| **即時通訊** | 低 | 聊天介面、訊息列表、表情符號 | Card, Form, Menu, Badge | 高 |
...
**任務**：
1. 性能優化（虛擬滾動、懶加載）
2. 文檔完善（Storybook、範例程式碼）
3. 測試覆蓋（單元測試、整合測試）
4. 無障礙支援（ARIA 標籤、鍵盤導航）
...
網站使用 Tairitsu 的 Routable derive 宏。
...
Hikari 的三層次組件體系提供了清晰的開發路線：

1. **Layer 1** - 原子級組件，高度可復用
2. **Layer 2** - 複合組件，常見 UI 模式
3. **Layer 3** - 生產級組件，完整業務功能

通過漸進式增強，從簡單到複雜，確保每一層都有完整的測試和文檔，為生產環境提供可靠的組件體系。
