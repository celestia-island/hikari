//! GlowWrapper — mouse-following spotlight effect for interactive components.
//!
//! Renders a wrapper `<div class="hi-glow-wrapper">` that tracks mouse position
//! via CSS custom properties (`--glow-x`, `--glow-y`) and shows a radial-gradient
//! spotlight on hover through a `::before` pseudo-element.
//!
//! Based on hikari-legacy's Glow component, adapted for tairitsu_vdom.
...
impl GlowColor {
    fn css_var(&self) -> &'static str {
        match self {
            Self::Primary => "var(--hi-glow-button-primary, rgba(245,169,169,0.20))",
            Self::Ghost => "var(--hi-ghost-glow, rgba(128,128,128,0.15))",
            Self::Secondary => "var(--hi-glow-button-secondary, rgba(81,154,115,0.15))",
            Self::Danger => "var(--hi-glow-button-danger, rgba(239,68,68,0.15))",
            Self::Success => "var(--hi-glow-button-success, rgba(34,197,94,0.15))",
        }
    }
}
...
pub fn glow_wrap(children: VNode, config: GlowConfig) -> VNode {
    ...
    let classes =
        format!("hi-glow-wrapper{block_class} hi-glow-blur-medium {intensity_class}{extra}");
    ...
    let onmouseenter = move |e: std::boxed::Box<dyn tairitsu_vdom::EventData>| {
        if let Some(me) = e.as_any().downcast_ref::<tairitsu_vdom::MouseEvent>() {
            if let Some(target) = me.target {
                set_style(target, "--glow-visible", "1");
            }
        }
    };
    ...
    let onmouseleave = move |e: std::boxed::Box<dyn tairitsu_vdom::EventData>| {
        if let Some(me) = e.as_any().downcast_ref::<tairitsu_vdom::MouseEvent>() {
            if let Some(target) = me.target {
                ...
                set_style(target, "--glow-visible", "0");
            }
        }
    };
