...
fn register_wasm(platform: &tairitsu_web::WitPlatform) {
    use hikari_components::platform::{
        ContentEditableState, DomQueryOps, EditableOps, HikariPlatform, MediaOps, ObserverOps,
        TimerOps, ViewportOps,
    };
    use tairitsu_vdom::platform::{
        CanvasOps, ClipboardOps, ContentEditableOps, DomOps, LayoutOps, MediaOps as _,
        ObserverOps as _, QueryOps, ScrollOps, TimerOps as _,
    };
    use tairitsu_web::WitElement;

    let hp = HikariPlatform {
        viewport: ViewportOps {
        ...
        observers: ObserverOps {
            ...
            observe_mutations_element: Box::new(
                |_observer, _element, _cl, _attr, _cd, _subtree| {},
            ),
