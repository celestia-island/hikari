...
pub fn custom(f: fn(f64) -> f64) -> EasingFunction {
    EasingFunction::Custom(Rc::new(f))
}
...
pub fn bezier(x1: f64, y1: f64, x2: f64, y2: f64) -> EasingFunction {
    EasingFunction::Custom(Rc::new(move |t| cubic_bezier(t, x1, y1, x2, y2)))
}
...
pub fn steps(n: u32, start: bool) -> EasingFunction {
    EasingFunction::Custom(Rc::new(move |t| {
        if t <= 0.0 {
...
pub fn power(p: f64) -> EasingFunction {
    EasingFunction::Custom(Rc::new(move |t| t.powf(p)))
}
