...
/// Builder for creating timelines with fluent API
...
impl TimelineBuilder {
    ...
    pub fn with_repeat(mut self, count: Option<u32>) -> Self {
        self.timeline.set_repeat(count);
        self
    }

    /// Enable or disable yoyo
    ///
    /// # Arguments
    /// * `enabled` - Whether to enable yoyo
    pub fn with_yoyo(mut self, enabled: bool) -> Self {
        self.timeline.set_yoyo(enabled);
        self
    }
    ...
    /// * `tween` - The tween to add
    /// * `start_time` - Time at which the tween should start
    pub fn add_tween(mut self, tween: Tween, start_time: Duration) -> Self {
        self.timeline.add_tween(tween, start_time);
        self
    }
    ...
    /// # Arguments
    /// * `tween` - The tween to add
    pub fn add_tween_at(mut self, tween: Tween) -> Self {
        self.timeline.add_tween_at(tween);
        self
    }
...
/// Extension trait for timeline conversions
pub trait TimelineExt {
    /// Convert to timeline
    fn to_timeline(&self) -> Timeline;
}

impl TimelineExt for Timeline {
    fn to_timeline(&self) -> Timeline {
        self.clone()
    }
}
