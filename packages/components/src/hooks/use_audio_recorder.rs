...
pub fn use_audio_recorder() -> (
    ...
    // Track processed final results to avoid duplicates (moved to top level)
    let processed_indices = use_signal(|| std::collections::HashSet::<(u32, u32)>::new());
        ...
        Callback::new(move |_| {
            #[cfg(target_arch = "wasm32")]
            {
                ...
                if !is_audio_recording_supported() {
                    state.set(AudioRecorderState::Error(
                            ...
                            let success_callback = {
                                ...
                                Closure::wrap(Box::new(move |stream: JsValue| {
                                    ...
                                    let audio_ctx_ctor = Reflect::get(&win, &"AudioContext".into())
                                        .or_else(|_| {
                                            Reflect::get(&win, &"webkitAudioContext".into())
                                        })
                                        .ok();
                                    ...
                                    if let Some(ctor) = audio_ctx_ctor {
                                        ...
                                        if let Ok(audio_context) =
                                            Reflect::construct(&ctor_fn, &Array::new())
                                        {
                                            ...
                                            // Create analyser
                                            if let Some(create_analyser) = Reflect::get(
                                                &audio_context,
                                                &"createAnalyser".into(),
                                            )
                                            .ok()
                                            {
                                                let create_fn: Function =
                                                    create_analyser.unchecked_into();
                                                if let Ok(analyser) =
                                                    create_fn.call0(&audio_context)
                                                {
                                                    // Configure analyser
                                                    Reflect::set(
                                                        &analyser,
                                                    ...
                                                    .ok()
                                                    {
                                                        ...
                                                        if let Ok(source) =
                                                            create_fn.call1(&audio_context, &stream)
                                                        {
                                                            // Connect source to analyser
                                                            if let Some(connect) = Reflect::get(
                                                                &source,
                                                                &"connect".into(),
                                                            )
                                                            .ok()
                                                            {
                                                                let connect_fn: Function =
                                                                    connect.unchecked_into();
                                                                connect_fn
                                                                    .call1(&source, &analyser)
                                                                    .ok();
                                                            }

                                                            state
                                                                .set(AudioRecorderState::Recording);
                                                            web_sys::console::log_1(&"[use_audio_recorder] Recording started, state set to Recording".into());

                                                            // Create visualization loop
                                                            let visualize = {
                                                                let mut audio_levels =
                                                                    audio_levels.clone();
                                                                let mut animation_frame_id =
                                                                    animation_frame_id.clone();
                                                                let win = win.clone();
                                                                ...
                                                                Closure::wrap(Box::new(move || {
                                                                        ...
                                                                        .ok()
                                                                    {
                                                                        ...
                                                                        const NOISE_FLOOR: f32 =
                                                                            0.05; // Ignore very low noise
                                    ...
                                    let speech_recognition =
                                        Reflect::get(&win, &"SpeechRecognition".into())
                                            .or_else(|_| {
                                                Reflect::get(
                                                    &win,
                                    ...
                                    if let Some(sr) = speech_recognition {
                                            ...
                                            Reflect::construct(&constructor, &Array::new())
                                        {
                                            ...
                                            // Uses deduplication to avoid repeating the same text
                                            let on_result = {
                                                let mut transcript = transcript.clone();
                                                let mut final_transcript = final_transcript.clone();
                                                let mut processed_indices = processed_indices.clone();

                                                Closure::wrap(Box::new(move |event: JsValue| {
                                                    if let Some(results) =
                                                        Reflect::get(&event, &"results".into()).ok()
                                                    {
                                                        if let Some(length) =
                                                            Reflect::get(&results, &"length".into())
                                                                .ok()
                                                                .and_then(|l| l.as_f64())
                                                        {
                                                            ...
                                                            if length > 0 {
                                                                                ...
                                                                                .ok()
                                                                            {
                                                                                if let Some(text) = Reflect::get(&item, &"transcript".into()).ok().and_then(|t| t.as_string()) {
                                                                                    if is_final {
                                                                                        ...
                                                                                        let result_index = (i, text.len() as u32);
                                                                                        let mut processed = processed_indices();
                                                                                        if !processed.contains(&result_index) {
                                                                ...
                                                                // Accumulate new final text (only non-duplicates)
                                                                if !new_final.is_empty() {
                                                                    let mut current =
                                                                        final_transcript();
                                                                    // Simple deduplication: don't append if it ends with the same text
                                                                    if !current
                                                                        .ends_with(&new_final)
                                                                    {
                                                                        current
                                            ...
                                            // Start recognition
                                            if let Some(start) =
                                                Reflect::get(&recognition, &"start".into()).ok()
                                            {
        ...
        Callback::new(move |_| {
            #[cfg(target_arch = "wasm32")]
            {
                ...
                if let Some(id) = animation_frame_id() {
                    if let Some(win) = window() {
                        if let Some(cancel) =
                            Reflect::get(&win, &"cancelAnimationFrame".into()).ok()
                        {
                ...
                web_sys::console::log_1(
                    &"[use_audio_recorder] Recording stopped, state reset to Idle".into(),
                );
            }
