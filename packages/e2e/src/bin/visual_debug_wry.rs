...
use hikari_e2e::debug_client::{
    A11yNode, BatchOp, DebugClient, ErrorsResponse, NetworkResponse, PerformanceMetrics,
    ReadyResponse, ScreenshotRequest, ViewportResponse,
};
...
fn build_screenshot_req(
    ...
    let screenshot_mode = if pixel || mode == "pixel" {
        Some("pixel".into())
    } else {
        None
    };
    ScreenshotRequest {
        ...
        full_page: if full_page || mode == "full_page" {
            Some(true)
        } else {
            None
        },
...
async fn do_capture(
    ...
    let filename = match (&route, &selector) {
        (Some(r), Some(s)) => format!(
            "{}_{}.png",
            r.trim_start_matches('/').replace('/', "_"),
            s.replace(['#', '.', ' '], "_")
        ),
        (Some(r), None) => {
    ...
    println!(
        ...
        w,
        h,
        req.mode.as_deref().unwrap_or("canvas")
    );
    Ok(())
}
...
async fn do_batch(
    ...
    let routes: Vec<(String, String)> = match routes_opt {
        Some(s) => s
            ...
            .filter(|r| !r.is_empty())
            .map(|r| (r.clone(), r.trim_start_matches('/').replace('/', "_")))
            .collect(),
        None => PREDEFINED_ROUTES
            .iter()
    ...
    for (url, name) in &routes {
        ...
        match do_capture(client, out, "full_page", Some(url), None, true, pixel).await {
            ...
            Err(e) => {
                ...
                results.push(
                    json!({"route": url, "name": name, "status": "error", "error": e.to_string()}),
                );
            }
        }
    }
...
async fn do_inspect(
    ...
    if let Some(sel) = selector {
        ...
        if let Some(t) = &dom.text {
            println!("  Text: {}", t);
        }
        println!(
            "  Visible: {}, Count: {}",
            dom.visible.unwrap_or(false),
            dom.count
        );
        if let Some(rect) = &dom.rect {
            ...
            if let Some(cv) = rect.children_visible {
                println!("  Children visible: {}", cv);
            }
            if let Some(ov) = rect.overflowing {
                println!("  Overflowing: {}", ov);
            }
        }
        ...
        println!(
            "  Engine: {} | Connected: {}",
            info.browser_engine, info.browser_connected
        );
        ...
        println!(
            "  Ready: {} | WASM: {} | Hydrated: {}",
            ready.wasm_loaded, ready.wasm_loaded, ready.hydrated
        );
        ...
        println!(
            "  {}x{} (dpr: {:.1})",
            vp.width, vp.height, vp.device_pixel_ratio
        );
        ...
        println!(
            "\n── Errors ({}) / Rejections ({}) ──",
            errors.errors.len(),
            errors.unhandled_rejections.len()
        );
        for err in &errors.errors {
            println!(
                ...
                err.error_type,
                err.message,
                err.stack.as_deref().map(|s| &s[..s.len().min(80)])
            );
        }

        let console = client
            .console_filtered(Some("error,warn"), None, Some(20))
            .await?;
...
async fn do_interactive(client: &DebugClient, route: &str) -> Result<()> {
    ...
    if errs.errors.is_empty() && errs.unhandled_rejections.is_empty() {
        ...
        for e in &errs.unhandled_rejections {
            println!("  [rejection] {}", e.message);
        }
    }
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Batch {
            routes,
            output_json,
        } => do_batch(&client, &out, routes.as_ref(), output_json, cli.pixel).await?,
        Commands::Inspect {
            route,
            selector,
            computed,
        } => do_inspect(&client, route.as_ref(), selector.as_ref(), computed).await?,
        ...
        Commands::Ready => {
            ...
            match client.ready().await {
                Ok(r) => println!(
                    "ready={} wasm={} hydrated={}",
                    r.ready, r.wasm_loaded, r.hydrated
                ),
        ...
        Commands::Errors => {
            ...
            println!(
                "Errors: {} | Rejections: {}",
                errs.errors.len(),
                errs.unhandled_rejections.len()
            );
            for e in &errs.errors {
                println!("  [{}] {}", e.error_type, e.message);
            }
            for e in &errs.unhandled_rejections {
                println!("  [rej] {}", e.message);
            }
        }
        Commands::Perf => {
            ...
            if let Some(dc) = p.dom_complete_ms {
                println!("  DomComplete: {:.0}ms", dc);
            }
            if let Some(fcp) = p.fcp_ms {
                println!("  FCP: {:.0}ms", fcp);
            }
            if let Some(h) = p.js_heap_used_mb {
                println!("  JS Heap: {:.1}MB", h);
            }
            println!(
                "  WASM: {} | Hydrated: {} | Timestamp: {}",
                p.wasm_loaded, p.hydrated, p.timestamp
            );
        }
        Commands::Network => {
        ...
        Commands::BatchRoutes => {
            fs::create_dir_all(&out)?;
            let routes: Vec<BatchOp> = PREDEFINED_ROUTES
                .iter()
                .map(|(url, _name)| BatchOp::Navigate {
                    url: url.to_string(),
                    wait_for: Some("hydration".into()),
                })
                .chain(
                    PREDEFINED_ROUTES
                        .iter()
                        .map(|(_url, name)| BatchOp::Screenshot {
                            selector: None,
                            full_page: Some(true),
                            mode: Some("pixel".into()),
                            name: Some(format!("{}_pixel", name)),
                        }),
                )
                .collect();
            ...
            for r in &result.results {
                ...
                println!(
                    "  [{:6.0}ms] {} {} - {}",
                    r.duration_ms, status, r.op_type, r.name
                );
