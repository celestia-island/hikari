...
pub fn register_default_plugins() {
    let plugins: Vec<RegistryEntry> = vec![
        RegistryEntry {
    ...
    for entry in plugins {
        if let Err(e) = register_node_plugin(entry) {
            eprintln!("[PLUGINS] Warning: {e}");
        }
    }
}
...
static PLUGIN_INIT: LazyLock<()> = LazyLock::new(register_default_plugins);

pub fn init() {
    LazyLock::force(&PLUGIN_INIT);
}
