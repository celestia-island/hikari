...
//!
//! This crate provides the icon type definitions, SVG rendering macros,
//! and the icon lookup API. Actual icon path data is injected at build
//! time by `tairitsu-packager`.
//!
//! ## Usage
//!
//! ```rust,ignore
...
// ── icon data types (stable, not generated) ──
...
// ── generated icon data (injected by tairitsu-packager via TAIRITSU_ICONS_RS) ──
mod mdi_generated {
    include!(concat!(env!("OUT_DIR"), "/mdi_generated.rs"));
}
pub use mdi_generated::get;
...
/// Declarative macro to define icon data constants and the `get()` lookup.
///
/// Generated by `tairitsu-packager` during `tairitsu check --icons`.
///
/// ```ignore
/// impl_icons! {
///     "home" => { ident: HOME, path: "M10,20V14...", view_box: "0 0 24 24" },
///     "cog"  => { ident: COG,  path: "M19.14...",   view_box: "0 0 24 24" },
/// }
/// ```
#[macro_export]
macro_rules! impl_icons {
    ($($name:expr => { ident: $ident:ident, path: $path:expr, view_box: $vb:expr $(,)? }),* $(,)?) => {
        pub mod data {
            use crate::IconData;
            $(
                pub const $ident: IconData = IconData {
                    view_box: Some($vb),
                    width: Some("24"),
                    height: Some("24"),
                    path: Some($path),
        ...
        pub fn get(name: &str) -> Option<&'static crate::IconData> {
            match name {
                $(
                    $name => Some(&data::$ident),
                )*
                _ => None,
            }
        }
